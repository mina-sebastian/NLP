...
mapped_texts = list(map(repl_some, input_texts))
...
def reconstruct_entities(roner_output):
    result = []
    temp = []

    for ent in roner_output:
        if not ent['multi_word_entity']:
            if temp:
                result.append(" ".join([t['text'] for t in temp]))
                temp = []
...
print("Reconstructed entities:")
for output_text in output_texts:
    reconstructed_entities = reconstruct_entities(output_text['words'])
    print(reconstructed_entities)